...
1.6.3 - Compatibility with upcoming Unity versions! Additional bug fixes to brighten your day!

・[PRO] Fix FFT Audio Analysis occasionally failing to generate Spectrum Payloads due to attempts to read beyond the end of the audio file during Payload processing.
・[PRO] Fix a static property in the Master Audio integration's MasterAudioSuperVisor static property to actually be static.
・[PRO] Fix handling of Master Audio Dynamic Sound Groups.
・[PRO] Fix handling of AudioClips in builds when Sample Rate differs from the authored value when using the Master Audio integration.
・Improve initial update handling when [re]starting playback.
・Fix incompatibility between packaged versions of Koreographer and Unity 2023.2.0+.
・Fix AudioClip related warnings when opening an empty Koreography Editor window in recent versions of Unity (e.g. Unity 6).
・Fix obsolete API usage in a demo script on Unity 6+.
・Fix deprecated API usage in Unity 2023.1.0+.

1.6.2 - Sample Rate-independent playback in builds! Some bug fixes to help grease the wheels!
